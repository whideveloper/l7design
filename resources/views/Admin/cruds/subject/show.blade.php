        ...
        <div class="content">
            ...
            <div class="container-fluid">
                        ...
                        <div class="page-title-box">
                            <div class="page-title-right">
                                <ol class="breadcrumb m-0">
                                    ...
                                    <li class="breadcrumb-item"><a href="{{route('admin.dashboard.subject.index')}}">Matérias</a></li>
                ...
                <div class="row gap-3 justify-content-end">
                    @if (Auth::user()->hasRole('Super') || Auth::user()->hasRole('Administrador'))
                        <div class="col-4 mb-3 row gap-2 align-content-md-end">
                            <button id="btSubmitDeleteForever" data-route="{{route('admin.dashboard.subject.destroySelectedForced')}}" type="button" class="btn btn-danger ms-2" style="display: none;width:170px;">Deletar registros <i class="mdi mdi-delete-restore"></i></button>

                            <button id="btSubmitRestore" data-route="{{route('admin.dashboard.subject.retoreDataAll')}}" type="button" class="btn btn-primary" style="display: none;width:170px;">Restaurar registros <i class="mdi mdi-restore"></i></button>
                        </div>
                    @endif
                    <div class="col-8 p-0 d-flex flex-wrap">
                        <form action="{{route('admin.dashboard.subject.show.search')}}" method="POST" class="row justify-content-end col-12">
                            @csrf
                ...
                <div class="row col-lg-12 d-flex justify-content-end mb-3 ms-0">
                    @if (route('admin.dashboard.subject.show.search') == url()->current())
                        <a href="{{route('admin.dashboard.subject.show')}}" class="btn btn-primary" style="width: 110px;height: 38px;margin-top: 0px;">Limpar Filtro</a>
                    @endif
                </div>
                ...
                <table data-toggle="table" data-page-size="5" data-pagination="false" class="table-bordered table-sortable">
                    ...
                    <tbody data-route="{{route('admin.dashboard.subject.sorting')}}">
                        @foreach ($subjectsDeleted_at as $key => $subject)
                            <tr data-code="{{$subject->id}}">
                                ...
                                <td>
                                    <div class="row">
                                        @can('usuario.editar')
                                            <div class="col-4">
                                                <form action="{{route('admin.dashboard.subject.retoreData',['subject' => $subject->id])}}" class="col-4" method="POST">
                                                    @csrf
                                                    @method('POST')
                                        ...
                                        @endcan
                                        @can('usuario.remover')
                                            <form action="{{route('admin.dashboard.subject.deleteForced',['subject' => $subject->id])}}" class="col-4" method="POST">
                                                @csrf
                                                @method('DELETE')
                ...
                <div class="mt-3 float-end">
                    {{$subjectsDeleted_at->links()}}
                </div>
            </div> <!-- container -->
        </div> <!-- content -->
    </div>
    <script>
        var indexRoute = "{{ route('admin.dashboard.subject.index') }}";
    </script>
